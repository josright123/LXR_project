...
static int dm9051_dbg_fcr(struct board_info *db, char *head)
{
	struct net_device *ndev = db->ndev;
	unsigned int rb;
	int ret;
	
	ret = regmap_read(db->regmap_dm, DM9051_FCR, &rb);
	if (ret < 0) {
		netif_err(db, drv, ndev, "%s: error %d dbg read reg %02x\n",
			  __func__, ret, DM9051_FCR);
		return ret;
	}
				  
	printk("After %s, fcr %02lx\n", head, rb & GENMASK(7, 0));
	
	if ((rb & GENMASK(7, 0)) & 0x21) {
		...
		if (ret)
			return ret;
		printk("Read ANAR %04x\n", val & 0xffff);
		
		if (!(val & 0x400)) {
			uval = val & 0xffff;
			uval |= 0x400;
			ret = dm9051_phywrite(db, regnum, uval);
			if (ret)
				return ret;
...
static int dm9051_set_fcr(struct board_info *db)
{
	...
	ret = dm9051_set_reg(db, DM9051_FCR, fcr);
	dm9051_dbg_fcr(db, "set_reg");
	return ret;
}
...
static int dm9051_update_fcr(struct board_info *db)
{
	...
	ret = dm9051_update_bits(db, DM9051_FCR, FCR_RXTX_BITS, fcr);
	dm9051_dbg_fcr(db, "update_bits");
	return ret;
}
